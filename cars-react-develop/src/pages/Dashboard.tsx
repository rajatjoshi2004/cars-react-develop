...
const Dashboard: React.FC = () => {
  ...
  const formatWishlistData = (
    ...
    return data.map((item) => {
      ...
      // Get odometer values
      const odometerMi =
        lot && lot.odometer ? lot.odometer?.mi?.toLocaleString() : "N/A";
      const odometerKm =
        lot && lot.odometer ? lot.odometer?.km?.toLocaleString() : "N/A";
      ...
      return {
        ...
        odometer:
          odometerMi !== "N/A" ? `${odometerMi} mi (${odometerKm} km)` : "N/A",
        bid: lot ? lot.bid : "N/A",
        saleDate: saleDate,
        color: colorName,
        originalItem: item,
  ...
  const columns = [
    {
      ...
      render: (text: string, record: any) => (
        <Link to={`/cars/${record.vin}`}>
          <img src={text} alt="car" className="w-[150px] h-full object-cover" />
        </Link>
      ),
    },
    {
      ...
      render: (text: string, record: FormattedWishlistItem) => (
        <div>
          <div className="flex items-center">
            ...
            <img
              src={
                record?.lots?.[0].domain?.name === "copart_com" ? copart : aiia
              }
              alt={`${
                record?.lots?.[0].domain === "copart_com" ? "copart" : "aiia"
              }`}
              width={record?.lots?.[0].domain === "copart_com" ? 20 : 30}
            />
          </div>
          <br />
      ...
      <Content style={{ padding: 24 }}>
        ...
        >
          <Title level={3} style={{ marginBottom: 16 }}>
            {t("header.watchlist")}({wishlist.length})
          </Title>
          ...
          {!screens.xs ? (
            ...
            wishlist.map((item) => (
              <Card key={item.key} style={{ marginBottom: 16 }}>
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      ...
                      <img
                        src={
                          item?.lots?.[0].domain?.name === "copart_com"
                            ? copart
                            : aiia
                        }
                        alt={`${
                          item?.lots?.[0].domain === "copart_com"
                            ? "copart"
                            : "aiia"
                        }`}
                        width={
                          item?.lots?.[0].domain === "copart_com" ? 20 : 30
                        }
                      />
                    </Link>
                    <br />
                ...
                <div>
                  <Text strong>{t("header.damage")}: </Text>
                  {item.damage}
                </div>
                <div>
                ...
                <div>
                  <Text strong>{t("header.currentBid")}:</Text>${item.bid} USD
                </div>

                <div>
                  <Text strong>{t("header.saleDate")}: </Text>
                  {item.saleDate}
                </div>
                <div>
                  <Text strong>{t("header.color")}: </Text>
                  {item.color}
                </div>
                <Button
                  type="primary"
