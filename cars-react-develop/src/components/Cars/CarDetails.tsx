...
const CarDetails = () => {
  ...
  const fetchCarData = async () => {
    ...
    try {
      ...
      if (res.data && res.data?.data) {
        ...
        if (res?.data?.data?.lots && res?.data?.data?.lots?.length > 0) {
  ...
  const formatImageGallery = (activeLot: any) => {
    ...
    if (!activeLot || !activeLot.images || !activeLot.images.normal) {
      ...
      data = activeLot.images.normal.map((imgUrl: any, index: number) => ({
        original: imgUrl,
        thumbnail: activeLot.images?.small?.[index] || imgUrl,
        originalAlt: `${carData?.title || "Vehicle"} image ${index + 1}`,
        thumbnailAlt: `${carData?.title || "Vehicle"} thumbnail ${index + 1}`,
      }));
    }
    setImgList(data);
  };
        ...
        <div className="w-full sm:w-3/5 lg:w-2/5 h-auto">
          ...
          <div className="hidden sm:block lg:hidden w-full border rounded-lg p-3 border-slate-300">
            <h3 className="text-center font-bold mb-4">
              {t("carDetails.auDetails")}
              {"   "}
              <img
                src={activeLot.domain.name === "copart_com" ? copart : aiia}
                alt={`${
                  activeLot.domain.name === "copart_com" ? "copart" : "aiia"
                }`}
                width={activeLot.domain.name === "copart_com" ? 20 : 30}
        ...
        <div className="w-full sm:w-2/5 lg:w-3/5 flex lg:flex-row flex-col gap-2">
          ...
          <div className="w-full lg:w-1/2 border rounded-lg p-3 border-slate-300">
            <div className="flex items-center">
              <h3 className="mb-0">{t("carDetails.auDetails")}</h3>
              <img
                src={activeLot.domain.name === "copart_com" ? copart : aiia}
                alt={`${
                  activeLot.domain.name === "copart_com" ? "copart" : "aiia"
    ...
    <>
      {carData ? (
        <div>
          ...
          {/* Vin Number */}
          <p className="flex items-baseline py-2 mb-0">
            <span className="font-light w-1/3">{t("carDetails.vin")}:</span>
            <span className="font-bold w-2/3">{carData.vin || "N/A"}</span>
          </p>
          <hr />
          ...
          <p className="flex items-baseline py-2 mb-0">
            <span className="font-light w-1/3">
              {t("carDetails.titleCode")}:
            </span>
            <span className="font-bold w-2/3">
              {activeLot.detailed_title?.name || activeLot.title?.name || "N/A"}
            </span>
          </p>
          <hr />
          ...
          {/* Secondary Damage */}
          <p className="flex items-baseline py-2 mb-0">
            <span className="font-light w-1/3">{t("carDetails.sDamage")}:</span>
            <span className="font-bold w-2/3">
          ...
          {/* Drive Wheel */}
          <p className="flex items-baseline py-2 mb-0">
            <span className="font-light w-1/3">{t("carDetails.dWheel")}: </span>
            <span className="font-bold w-2/3">
          ...
          {/* Keys */}
          <p className="flex items-baseline py-2 mb-0">
            <span className="font-light w-1/3">{t("carDetails.keys")}:</span>
            <span className="font-bold w-2/3">
              {activeLot?.keys_available ? "Available" : "Not Available"}
            </span>
          </p>
          <hr />
          ...
          {/* Year of Production */}
          <p className="flex items-baseline py-2 mb-0">
            <span className="font-light w-1/3">{t('carDetails.yrofproduction')}: </span>
            <span className="font-bold w-2/3">{carData?.year || "N/A"}</span>
          </p>
          <hr />

          {/* Special Note */}
          <p className="flex items-baseline py-2 mb-0">
            <span className="font-light w-1/3">{t('carDetails.spNotes')}:</span>
            <span className="font-bold w-2/3">
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 items-baseline py-1">
        <span className="font-light w-1/3">{t("header.currentBid")}:</span>
      ...
      <div className="py-1 gap-2 items-baseline flex">
        <span className="font-light w-1/3">{t("carDetails.timeLeft")}:</span>
...
const SaleInformation = ({ activeLot }: { activeLot: Lot }) => {
  ...
  return (
    <div>
      ...
      >
        <span className="font-light w-1/3">{t("carDetails.vehicalLoc")}:</span>
        <span className="font-bold w-2/3">
      ...
      {/* Auction Date */}
      <p className="flex items-baseline py-2 mb-0">
        <span className="font-light w-1/3">{t("carDetails.auDate")}:</span>
        <span className="font-bold w-2/3">
          {activeLot?.sale_date
            ? moment(activeLot?.sale_date)?.format("DD-MM-YYYY, HH:mmA")
            : "N/A"}
        </span>
      </p>
      <hr />
      {/* Actual Cash Value */}
      <p className="flex items-baseline py-2 mb-0">
        <span className="font-light w-1/3">{t("carDetails.cashValue")}:</span>
        <span className="font-bold w-2/3">
          $
      ...
      <div className="border mt-2 h-fit rounded-lg p-3 border-slate-300">
        <h3 className="font-semibold">{t("carDetails.pCalculator")}</h3>
